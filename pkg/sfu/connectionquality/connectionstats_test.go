...
func newConnectionStats(mimeType string) *ConnectionStats {
...
func TestConnectionQuality(t *testing.T) {
	t.Run("audio quality - opus - no fec", func(t *testing.T) {
		cs := newConnectionStats("audio/opus")
		cs.Start(&livekit.TrackInfo{Type: livekit.TrackType_AUDIO})

		// no data should return default state which is EXCELLENT quality
		cs.updateScore(nil)
		mos, quality := cs.GetScoreAndQuality()
		require.GreaterOrEqual(t, MaxMOS, mos)
