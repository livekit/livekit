...
type EndpointsConfig struct {
	...
	// RelayPort enables multi-node (and, on a bus spanning regions,
	// cross-region) serving: the node answers endpoint resolves on the message
	// bus and accepts relayed requests on this private listener. 0 disables it
	// (single node).
