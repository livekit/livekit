...
func RecordTwirpRequestLatency(service, method string, duration time.Duration, statusFamily string, code twirp.ErrorCode) {
	promTwirpRequestLatency.WithLabelValues(service, method, statusFamily, string(code)).Observe(float64(duration.Milliseconds()))
}
