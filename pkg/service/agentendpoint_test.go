...
// --- multi-node: resolve over the bus, relay over HTTP, locality wins ---

type multiNodeStack struct {
	*endpointStack
	relayTS *httptest.Server
}

func newMultiNodeStack(t *testing.T, bus psrpc.MessageBus, nodeID string) *multiNodeStack {
	s := newEndpointStack(t, agent.EndpointsConfig{})
	relayTS := httptest.NewServer(s.svc.EndpointRelayHandler())
	t.Cleanup(relayTS.Close)
	require.NoError(t, s.svc.EnableEndpointRemote(bus, nodeID, relayTS.URL))
	return &multiNodeStack{endpointStack: s, relayTS: relayTS}
}

func TestAgentEndpointsMultiNode(t *testing.T) {
	bus := psrpc.NewLocalMessageBus()
	nodeA := newMultiNodeStack(t, bus, "node-a")
	nodeB := newMultiNodeStack(t, bus, "node-b")

	mkApp := func(name string) *httptest.Server {
		mux := http.NewServeMux()
		mux.HandleFunc("GET /json", func(w http.ResponseWriter, r *http.Request) {
			_, _ = w.Write([]byte(name))
		})
		mux.HandleFunc("POST /sms", func(w http.ResponseWriter, r *http.Request) {
		...
		return newTargetApp(t, mux)
	}

	eps := []*livekit.AgentHttp_AgentEndpoint{
		httpEP("/json", []string{"GET"}, true),
		httpEP("/stream", []string{"GET"}, true),
		httpEP("/sms", []string{"POST"}, true),
	}

	// the only worker attaches to node A; every request into node B must
	// resolve over the bus and relay over HTTP
	workerA := nodeA.startWorker(mkApp("A").URL, "production", eps)

	t.Run("request into the other node relays", func(t *testing.T) {
		resp, err := http.Get(nodeB.ts.URL + "/agents/production/json")
		require.NoError(t, err)
		body, _ := io.ReadAll(resp.Body)
		resp.Body.Close()
		require.Equal(t, 200, resp.StatusCode)
	...
	t.Run("streaming survives the relay hop", func(t *testing.T) {
		start := time.Now()
		resp, err := http.Get(nodeB.ts.URL + "/agents/production/stream")
		require.NoError(t, err)
		defer resp.Body.Close()
		br := bufio.NewReader(resp.Body)
		...
		require.Less(t, time.Since(start), 300*time.Millisecond,
			"first chunk must arrive before the stream completes (no buffering across the relay)")
		rest, _ := io.ReadAll(br)
		require.Contains(t, string(rest), "chunk-2")
	})

	t.Run("form POST body survives the relay", func(t *testing.T) {
		resp, err := http.Post(
			nodeB.ts.URL+"/agents/production/sms",
			"application/x-www-form-urlencoded",
			strings.NewReader("From=%2B15550001&Body=hi"),
		)
		require.NoError(t, err)
		body, _ := io.ReadAll(resp.Body)
		resp.Body.Close()
	...
	t.Run("404 for unknown paths resolves cleanly", func(t *testing.T) {
		resp, err := http.Get(nodeB.ts.URL + "/agents/production/nope")
		require.NoError(t, err)
		resp.Body.Close()
		require.Equal(t, 404, resp.StatusCode)
	})

	// a second worker on node B: local traffic stays local
	var _ = workerA
	nodeB.startWorker(mkApp("B").URL, "production", eps)

	t.Run("locality wins", func(t *testing.T) {
		for i := 0; i < 6; i++ {
			resp, err := http.Get(nodeB.ts.URL + "/agents/production/json")
			require.NoError(t, err)
			body, _ := io.ReadAll(resp.Body)
			resp.Body.Close()
			require.Equal(t, "B", string(body), "local candidates must serve without a bus round trip")
		}
	})
