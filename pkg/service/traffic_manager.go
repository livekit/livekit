...
func NewTrafficManager(db *pubsub.DB, logger logger.Logger) *TrafficManager {
	m := &TrafficManager{
		db:                    db,
		lock:                  sync.RWMutex{},
...
func (m *TrafficManager) init(ctx context.Context) error {
	...
	return m.db.Subscribe(ctx, topicTrafficValuesStreaming, func(event common.Event) {
		jsonMsg, ok := event.Message.(string)
		if !ok {
			m.logger.Errorw("convert interface to string from message topic traffic values", errors.New("convert interface to string from message topic traffic values"))
			return
		}
