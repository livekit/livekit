...
func (h *RelevantNodesHandler) HTTPHandlerNode(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		handleError(w, http.StatusBadRequest, fmt.Errorf("fetch relevant node %w", err))
		return
	}
...
func (h *RelevantNodesHandler) HTTPHandlerNodes(w http.ResponseWriter, r *http.Request) {
	var req SearchRelevantNodeRequest
	err := json.NewDecoder(r.Body).Decode(&req)
	if err != nil {
		handleError(w, http.StatusBadRequest, fmt.Errorf("unmarshal body %w", err))
	...
	nodes, err := h.nodeProvider.FetchRelevantNodes(r.Context(), req.IP)
	if err != nil {
