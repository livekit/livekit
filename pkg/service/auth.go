...
func (m *APIKeyAuthMiddleware) ServeHTTP(w http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
	...
	if authToken != "" {
		...
		if r.URL != nil && len(grants.Identity) > 0 && len(grants.Video.Room) > 0 {
			revoked, nbf, _ := m.tokenRevocationStore.IsRoomParticipentRevoked(ctx, livekit.ParticipantIdentity(grants.Identity), livekit.RoomName(grants.Video.Room))
			if revoked && claims.NotBefore.Time().Unix() < nbf.Time().Unix() {
