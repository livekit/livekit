...
type AgentHandler struct {
	...
	endpointRemote   *endpoint.Remote
...
func NewAgentService(
	...
	if port := conf.Agents.Endpoints.RelayPort; port > 0 {
		relayURL := fmt.Sprintf("http://%s:%d", currentNode.NodeIP(), port)
		if err := s.EnableEndpointRemote(bus, string(currentNode.NodeID()), relayURL); err != nil {
			return nil, err
		}
	}
	return s, nil
...
// EnableEndpointRemote turns on multi-node endpoint resolution: this node
// answers resolves for its registrations on the bus and relays requests it
// cannot serve to the winning node's relay listener.
func (s *AgentService) EnableEndpointRemote(bus psrpc.MessageBus, nodeID, relayURL string) error {
	remote, err := endpoint.NewRemote(s.endpointRegistry, bus, nodeID, relayURL, s.logger)
	if err != nil {
		return err
	}
	s.endpointRemote = remote
	return nil
}

// EndpointRelayHandler serves relayed requests from other nodes; bind it to a
// private address only, its trust model is the network.
func (s *AgentService) EndpointRelayHandler() http.Handler {
	return endpoint.NewRelayFront(s.endpointRegistry, s.logger)
}
...
func (s *AgentService) EndpointFront() http.Handler {
	...
	front.WithRemote(func() *endpoint.Remote { return s.endpointRemote })
