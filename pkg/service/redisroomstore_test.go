...
func TestSubscriptionPermissionPersistence(t *testing.T) {
	...
	// create the subscription permission
	require.NoError(t, rs.StoreSubscriptionPermission(ctx, roomName, "user1", sp))

	// result should match
	spGet, err := rs.LoadSubscriptionPermission(ctx, roomName, "user1")
	require.NoError(t, err)
	require.False(t, spGet.AllParticipants)
	require.Equal(t, len(sp.TrackPermissions), len(spGet.TrackPermissions))
	for i, trackPermission := range sp.TrackPermissions {
		require.Equal(t, spGet.TrackPermissions[i].ParticipantSid, trackPermission.ParticipantSid)
		for j, trackSid := range trackPermission.TrackSids {
			require.Equal(t, spGet.TrackPermissions[i].TrackSids[j], trackSid)
		}
	}
	...
	// update the subscription permission
	require.NoError(t, rs.StoreSubscriptionPermission(ctx, roomName, "user1", sp))

	// result should match
	spGet, err = rs.LoadSubscriptionPermission(ctx, roomName, "user1")
	require.NoError(t, err)
	require.True(t, spGet.AllParticipants)
	require.Nil(t, spGet.TrackPermissions)
	...
	require.NoError(t, err)
	require.False(t, spGet.AllParticipants)
	require.Equal(t, len(spGet.TrackPermissions), 0)

	// deleting subscription permission should return to normal
	require.NoError(t, rs.DeleteSubscriptionPermission(ctx, roomName, "user1"))
