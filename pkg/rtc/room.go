...
func NewRoom(room *livekit.Room, config WebRTCConfig, audioConfig *config.AudioConfig) *Room {
	r := &Room{
		...
		Logger:          logger.GetLogger().WithValues("room_name", room.Name),
		config:          config,
...
func (r *Room) Join(participant types.Participant, opts *ParticipantOptions, iceServers []*livekit.ICEServer) error {
	...
	participant.OnStateChange(func(p types.Participant, oldState livekit.ParticipantInfo_State) {
		logger.WithLogger(r.Logger).Debugw("participant state changed", "state", p.State(), "participant", p.Identity(), "pID", p.ID(),
			"oldState", oldState)
	...
	logger.WithLogger(r.Logger).Infow("new participant joined",
		"pID", participant.ID(),
...
func (r *Room) Close() {
	r.closeOnce.Do(func() {
		close(r.closed)
		logger.WithLogger(r.Logger).Infow("closing room", "roomID", r.Room.Sid, "room", r.Room.Name)
...
func (r *Room) SetMetadata(metadata string) {
	...
	for _, p := range r.GetParticipants() {
		...
		if err != nil {
			logger.WithLogger(r.Logger).Warnw("failed to send room update", err, "room", r.Room.Name, "participant", p.Identity())
		}
	}
...
func (r *Room) onTrackPublished(participant types.Participant, track types.PublishedTrack) {
	...
	for _, existingParticipant := range r.participants {
		...
		logger.WithLogger(r.Logger).Debugw("subscribing to new track",
		...
		if err := track.AddSubscriber(existingParticipant); err != nil {
			logger.WithLogger(r.Logger).Errorw("could not subscribe to remoteTrack", err,
				"participants", []string{participant.Identity(), existingParticipant.Identity()},
				"pIDs", []string{participant.ID(), existingParticipant.ID()},
				"track", track.ID())
		}
...
func (r *Room) subscribeToExistingTracks(p types.Participant) {
	...
	for _, op := range r.GetParticipants() {
		...
		if n, err := op.AddSubscriber(p); err != nil {
			...
			logger.WithLogger(r.Logger).Errorw("could not subscribe to participant", err,
	...
	if tracksAdded > 0 {
		logger.WithLogger(r.Logger).Debugw("subscribed participants to existing tracks", "tracks", tracksAdded)
	}
}
...
func (r *Room) broadcastParticipantState(p types.Participant, skipSource bool) {
	...
	if p.Hidden() {
		if !skipSource {
			...
			if err != nil {
				logger.WithLogger(r.Logger).Errorw("could not send update to participant", err,
