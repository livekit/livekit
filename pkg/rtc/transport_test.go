...
// sdpHasApplicationMedia checks whether an SDP contains an m=application media section.
// Returns true if a data channel media description is present, false otherwise.
...
// server is the offerer. Without subscriber data channels the offer omits m=application,
// Pion never starts SCTP, and clients that push SCTP INIT on that transport (e.g. esp_peer
// on ESP32) get no response.
func TestSubscriberTransportSCTPPublisherPrimary(t *testing.T) {
	...
	t.Run("offer omits m=application without subscriber data channels", func(t *testing.T) {
		...
		var capturedOffer atomic.Pointer[webrtc.SessionDescription]
		handler.OnOfferCalls(func(sd webrtc.SessionDescription, _ uint32, _ map[string]string) error {
			capturedOffer.Store(&sd)
			return nil
		})
		server.Negotiate(true)

		require.Eventually(t, func() bool {
			offer := capturedOffer.Load()
			return offer != nil && offer.SDP != ""
		}, 10*time.Second, 10*time.Millisecond, "subscriber offer not produced")
		require.False(t, sdpHasApplicationMedia(*capturedOffer.Load()))
	})
	...
	t.Run("offer includes m=application with subscriber data channels", func(t *testing.T) {
		...
		require.True(t, sdpHasApplicationMedia(*capturedOffer.Load()))
	})
