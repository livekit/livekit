...
// Only needed if some layer is paused, i.e. some committed quality is not HIGH.
func (d *dynacastManagerVideo) ResendCommittedQuality() {
	d.lock.Lock()
	defer d.lock.Unlock()

	for _, quality := range d.committedMaxSubscribedQuality {
		if quality != livekit.VideoQuality_HIGH {
			d.enqueueSubscribedQualityChange()
			return
		}
	}
